...
[sRO, sProj, ss, nCh] = size(matrice);
matrice = reshape(matrice,1,sRO,sProj,nCh);

dcf = repmat(acquisition.bits.buffer.density,[1 1 1 nCh]);
%% create DCF for each channel :
% gridding
agrid = bart('nufft -a -c', traj,matrice); % blurring due to oversampling
agrid = bart('nufft -a -c', traj,matrice.*dcf); % if we have dcf we can correct that

igrid = bart('nufft -i -c', traj,matrice); % iterative reconstruction 

% sens with ecalib
ksp = bart('fft -u 3', igrid);
